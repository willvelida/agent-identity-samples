...
| [Call Azure Service with Entra Agent Identity](call-azure-service/) | An interactive chat application powered by Azure OpenAI that uses an Entra Agent Identity to autonomously authenticate to Azure Cosmos DB under the agent's own identity. Demonstrates the autonomous agent token pattern with a Blazor frontend and .NET API on Azure Container Apps. |
